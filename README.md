...
뉴모피즘 스타일 UI를 공통 테마로 사용하며, 무료일 때는 로컬 저장, 프리미엄일 때는 로컬 우선 저장과 선택적 원격 동기화를 지원합니다.
...
### 6) 데이터 구조
- `expo-sqlite` 기반 로컬 저장 (무료 플랜 기본)
- 동기화 큐로 로컬 변경사항(생성/수정/삭제) 순차 처리
- 프리미엄 플랜에서 원격 동기화 provider (`custom` / `firebase` / `supabase`) 확장 구조 지원
